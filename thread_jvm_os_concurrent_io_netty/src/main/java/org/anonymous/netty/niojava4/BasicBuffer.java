 ...
 * <p>
 * 0 <= mark <= position <= limit <= cacpacity
 ...
 * 当 Buffer 输出数据结束后, 调用 clear 方法, 不是清空 Buffer 数据. 改变 position/limit 值, 为再次向
 * Buffer 装入数据做准备.
 * *
 ...
 * to zero.  </p></li>
 * ##################################
 * @see java.nio.Buffer#hasRemaining() position 与 limit 之间是否还有元素可供处理
 * Channel 通道
 * NIO 的通道类似于流, 但有些区别
