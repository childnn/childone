 ...
 * 　　mput locale-files 将本地主机中一批文件传送至远端主机
 * 　　dir 或 ls [remote-directory] [locale-file] 列出当前远端主机目录中的文件.如果有本地文件,就将结果写至本地文件
 ...
 * 　　4.append locale-file[remote-file]：将本地文件追加到远程系统主机，若未指定远程系统文件名，则使用本地文件名。
 ...
 * 　　17.dir[remote-dir][locale-file]：显示远程主机目录，并将结果存入本地文件。
 * 　　18.disconnection：同close。
 * 　　19.form format：将文件传输方式设置为format，缺省为file方式。
 * 　　20.get remote-file[locale-file]： 将远程主机的文件remote-file传至本地硬盘的local-file。
 ...
 * 　　27. ls[remote-dir][locale-file]：显示远程目录remote-dir， 并存入本地文件local-file。
 * 　　28.macdef macro-name：定义一个宏，遇到macdef下的空行时，宏定义结束。
 * 　　29.mdelete[remote-file]：删除远程主机文件。
 * 　　30.mdir remote-files locale-file：与dir类似，但可指定多个远程文件，如 ：mdir *.o.*.zipoutfile 。
 * 　　31.mget remote-files：传输多个远程文件。
 * 　　32.mkdir dir-name：在远程主机中建一目录。
 * 　　33.mls remote-file locale-file：同nlist，但可指定多个文件名。
 * 　　34.mode[modename]：将文件传输方式设置为modename， 缺省为stream方式。
 * 　　35.modtime file-name：显示远程主机文件的最后修改时间。
 * 　　36.mput locale-file：将多个文件传输至远程主机。
 * 　　37.newer file-name： 如果远程机中file-name的修改时间比本地硬盘同名文件的时间更近，则重传该文件。
 * 　　38.nlist[remote-dir][locale-file]：显示远程主机目录的文件清单，并存入本地硬盘的local-file。
 ...
 * 　　45.put locale-file[remote-file]：将本地文件local-file传送至远程主机。
 * 　　46.pwd：显示远程主机的当前工作目录。
 * 　　47.quit：同bye，退出ftp会话。
 * 　　48.quote arg1，arg2...：将参数逐字发至远程ftp服务器，如：quote syst.
 * 　　49.recv remote-file[locale-file]：同get。
 * 　　50.reget remote-file[locale-file]：类似于get， 但若local-file存在，则从上次传输中断处续传。
 ...
 * 　　58.send locale-file[remote-file]：同put。
