 ...
 * @version $Revision: 1.47 $ $Date: 2004/08/26 21:28:57 $
 * @deprecated
 */
...
final class HttpProcessor
     ...
     * @see #run
     * @see #await
     * #run 是作为 processor 中的新线程启动, 在 {@link HttpConnector#newProcessor()} 中已经启动 #start
     * processor 线程启动后, #run 方法一直等待 socket 分配, 直到分配成功
     *
     */
    synchronized void assign(Socket socket) {

        // Wait for the Processor to get the previous Socket
        // 当一个新的套接字被分配的时候，连接器线程调用 HttpProcessor 的
        // assign 方法。available 的值是 false，所以 while 循环给跳过，并且套接字给赋值给
        // HttpProcessor 实例的 socket 变量：
        // this.socket = socket;
        // 连接器线程把 available 设置为 true 并调用 notifyAll。
        // 这就唤醒了处理器线程(#run --> #await)，因为
        // available 为 true，所以程序控制跳出 while 循环：把实例的 socket 赋值给一个本地变量，并
        // 把 available 设置为 false，调用 notifyAll，返回最后需要进行处理的 socket。
        while (available) { // 初始值为 false
            try {
                wait();
     ...
     * processor 线程启动在 {@link HttpConnector#newProcessor()} #start
     * @see #run()
     * @see #assign
     */
    private synchronized Socket await() {
        ...
        // 刚开始线程启动, available = false
        // 刚开始的时候，当处理器线程刚启动的时候，available 为 false，线程在 while 循环里边
        // 等待。它将等待另一个线程调用 notify 或 notifyAll。这就是说，调
        // 用 wait 方法让处理器线程暂停，直到连接器线程调用 HttpProcessor 实例的 notifyAll 方法。
        // #assign
        // org.apache.catalina.connector.http.HttpConnector.run
    ...
    private void parseConnection(Socket socket)
        ...
        // 从套接字中获取到网络地址并把它赋予 HttpRequestImpl 对象
        // 检查是否使用代理并把套接字赋予请求对象
        if (debug >= 2)
    ...
    private void parseHeaders(SocketInputStream input)
        ...
        // while 循环，可以持续读取 HTTP 请求直到再也没有更多的头部
        // 可以读取到。while 循环首先调用请求对象的 allocateHeader 方法来获取一个空的 HttpHead 实例
        while (true) {
    ...
    private void process(Socket socket) {
        ...
        boolean ok = true; // 在处理过程中是否发现错误
        boolean finishResponse = true; // Response 接口中的 finishResponse 方法是否应该被调用
        ...
        try {
            // 注意的是，
            // SocketInputStream 的构造方法 同样传递了 从连接器 获得的缓冲 区大小，而 不是从
        ...
        keepAlive = true; // 连接是否是持久的

        // while 循环用来保持从输入流中读取，直到 HttpProcessor 被停止，一个异常被抛出或者连接给关闭为止。
        // stopped:  HttpProcessor 实例是否已经被连接器终止来确认 process 是否也应该停止
        while (!stopped && ok && keepAlive) {
            ...
            try {
                if (ok) {
                    // process 方法通过调用 parseConnection，parseRequest 和 parseHeaders 方法开始
                    // 解析前来的 HTTP 请求
                    parseConnection(socket);
                    parseRequest(input, output);
                    if (!request.getRequest().getProtocol().startsWith("HTTP/0"))
                        parseHeaders(input);
                    // 从 web 客户端过来的 HTTP 请求是否支持 HTTP 1.1
                    if (http11) {
     ...
     * @see #assign
     * @see HttpConnector#newProcessor() start the processor-thread
     * @see #start()
     */
    public void run() {
