...
public class SolarTermsUtil {
    ...
    private static String getTermInternal(int year, int mouth, int day) {
        if (year >= 1900 && year <= 2100) {
            ...
            } else if (day == Integer.parseInt(segmentTable[segmentOffset])
                    || day > Integer.parseInt(segmentTable[segmentOffset])
                    && day < Integer.parseInt(segmentTable[segmentOffset + 1])) {
                return TERMS[segment * 4 + segmentOffset];
            } else {
                return day != Integer.parseInt(segmentTable[segmentOffset + 1])
                        && day <= Integer.parseInt(segmentTable[segmentOffset + 1]) ? "" : TERMS[segment * 4 + segmentOffset + 1];
            }
        } else {
