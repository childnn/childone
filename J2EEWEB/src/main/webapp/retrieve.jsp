...
<form action="${pageContext.request.contextPath}/deleteSelected" method="post">

    <%--本页面显示所有联系人: ok--%>
    <table border="1xp solid black">
        <caption><h1>联系人</h1></caption>
        <tr>
            <td><input type="checkbox" class="checkbox" id="first" onclick="change(this)"/></td><!--全选-->
            <td>编号</td>
            <td>姓名</td>
            <td>性别</td>
            <td>年龄</td>
        ...
        <c:if test="${not empty list}"><%--非空校验--%>
            <c:forEach items="${list}" var="contact" varStatus="vs">
                ...
                <tr onmouseover="addColor(this)" onmouseout="removeColor(this)">
                    ...
                    <td>${contact.email}</td>
                    <td>
                        <a href="${pageContext.request.contextPath}/getUpdate?id=${contact.id}">update</a> <%--修改--%>
                        <a href="${pageContext.request.contextPath}/delete?id=${contact.id}">delete</a> <%--删除--%>
                    </td>
                </tr>
            </c:forEach>
        </c:if>
    ...
    <h3 align="center" style="color: red"><c:if
            test="${empty sessionScope.list}">联系人为空,点击添加按钮添加联系人</c:if></h3> <%--来自 DeleteAllServlet 的数据--%>
    ...
    <div> <%--检索查询: 包括模糊查询--%>
        <%--查询指定用户信息： 给定多个信息,可能查到多条数据
            关键: dao 层 的 sql 拼接
        --%>
        <input type="text" name="name" placeholder="姓名">
        <input type="text" name="sex" placeholder="性别">
        <input type="text" name="age" placeholder="年龄">
        <input type="text" name="province_id" placeholder="籍贯">
        <input type="text" name="qq" placeholder="QQ">
        <input type="text" name="email" placeholder="email">
        <input type="button" value="查询" id="bt"> <%--利用点击事件提交--%>
    </div>
